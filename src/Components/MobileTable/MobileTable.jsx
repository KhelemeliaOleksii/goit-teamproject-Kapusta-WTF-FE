...
function MobileTable() {
  ...
  const transactions = useSelector(transactionSelectors.getTransactionList); 
    ...
    dispatch(transactionOperations.getTransaction(date));
     ...
     const onToggleDeletion = (transaction) => {
    ...
    const onConfirmDeletion = () => {
        setDeletionModal(false);
        const transactionToDelete = transactions.find(item => item._id === transaction);
    ...
    return (
        <Section>
            {deletionModal && (
                <Modal handleClickYes={onConfirmDeletion} handleClickNo = {onCancelDeletion} onClose = {onToggleDeletion} message = "Ви впевнені?" />
            )}
                <ul className={s.list}>
                    {transactions.map(item => (
                        <div key={item._id} className={s.wrapper}>
                            <li className={s.item}>
                                ...
                                <div className={s.item__amountWrapper}>
                                    <p className={s.item__amount} style={
                                        item.transactionType === "expenses"
                                        ? {color: "#E7192E"} : {color: "#407946"}
                                    }>
                                        {item.transactionType === "expenses" ? `-${item.amount}.00 грн.` : `${item.amount}.00 грн.`}
                                    </p>
                                </div>
                                <Delete
